...
클라우드에서 **인증·권한·작업 전달·상태·이력을 통제하는 관제탑**이다. 실행 자체는 사용자
PC와 브라우저가 하고, 이 저장소는 무엇을 실행할지 정하고 결과를 기록한다.

**1. 인증과 소유권**

- Cognito 발급 JWT 검증과 사용자 매핑
- PC 등록 — 5분·1회용 등록 코드 발급, Ed25519 서명 검증, 기기 토큰 발급
- **작업이 소유자의 PC에만 전달되도록 DB 계층에서 강제한다.** 애플리케이션 검증만으로는
  결함 하나가 곧바로 타인의 PC로 작업이 새는 사고가 된다
- 타인의 자원을 조회하면 403이 아니라 404로 답한다 — 403은 그 식별자의 자원이 존재한다는
  사실을 노출한다

**2. 작업 원장**

- 모든 요청을 기록한다. **즉시 완료되는 요청도 예외로 두지 않는다** — 원장에서 빠지면
  이력이 실제 사용 내역과 어긋난다
- 상태 전이를 이벤트로 남겨 연결이 끊겨도 복구할 수 있게 한다
- 같은 요청이 두 번 오면 같은 작업을 돌려준다(멱등)
- 만료된 작업·전달을 배치가 정리한다

**3. 처리 경로와 실행 위치 결정**

- 처리 경로(`ProcessingRoute`)는 작업 유형이 상수로 갖는다 — 외부 API · 사용자 PC · 요약
- 실행 위치(`ExecutionTarget`)는 같은 유형이라도 상황에 따라 달라진다 —
  `BROWSER` · `RUNNER` · `BACKEND`
- **실행 위치는 요청에 실려 온 값을 믿지 않는다.** 그대로 믿으면 브라우저가 자기 실행
  결과를 PC 실행 결과인 것처럼 제출할 수 있다

**4. Agent WebSocket 게이트웨이**

- 등록된 PC와의 WSS 연결을 유지하고 작업을 전달한다(`/ws/agent`)
- Heartbeat 30초, 90초 미수신 시 오프라인 판정. 연결이 끊긴 기기는 스윕이 정리한다
- 기기당 동시 작업 1건

**하지 않는 것** — 파일 원문과 절대 경로를 보관하지 않는다. 요약 원문은 성공한 순간
삭제한다. 결과는 64KB를 넘지 않는다(DB 제약 + 실행기 전송 전 절단).
