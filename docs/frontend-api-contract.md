...
`/status` (시스템 상태)·`/file` (파일 검색)·`/weather` (날씨)가 종단까지 동작합니다.
`/summary` 는 LLM 연결이 남아 있어 아직 `LLM_NOT_READY` 로 마감됩니다. **화면은 네 개를 다
만들어 두셔도 됩니다** — 오류 코드로 구분해 안내만 다르게 하면 붙는 대로 그대로 동작합니다.
