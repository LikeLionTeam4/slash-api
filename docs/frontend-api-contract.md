...
>
> **그 PC 가 지금 받을 수 없을 때도 서버가 처리합니다.** 꺼져 있거나, 사용자가 작업 수신을
> 꺼 두었거나, 다른 작업을 실행 중인 경우입니다. `/file` 같은 PC 전용 작업은 이럴 때
> `WAITING_FOR_DEVICE` 로 기다리거나 `DEVICE_BUSY` 로 마감되지만, **요약은 서버가 대신할 수
> 있으므로 기다리게 하지도 실패시키지도 않습니다.** 어디서 실행됐는지는 결과의
> `executionTarget` 으로 확인하세요 — PC 를 골랐어도 `BACKEND` 로 올 수 있습니다.
>
...
- 결과에 남는 `executionTarget` 은 `BROWSER` 입니다. 결과 형식은 아래 §`TEXT_SUMMARY` 의
  세 번째 예시를 보세요.
- **이 Endpoint 는 승인 단계를 거치지 않습니다.** `WAITING_FOR_APPROVAL` 이 나오지 않으므로
  승인·거절 화면을 준비할 필요가 없습니다. 승인은 *실행하기 전에* 묻는 것인데 여기 오는
  것은 이미 브라우저에서 끝난 실행의 결과라 물어볼 시점이 지났기 때문입니다 — 나중에
  요약이 승인 대상이 되더라도 `POST /api/v1/requests` 로 보낸 요약만 멈춥니다.
