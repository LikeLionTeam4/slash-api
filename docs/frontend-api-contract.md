...
> `{"text": "/open <fileRef>"}` 로 보내시면 됩니다.
>
> `fileRef` 는 **뜻을 알 수 없는 문자열**입니다. 절대 경로는 클라우드로 오지 않고, 그 값으로
> 실제 경로를 되찾는 일은 PC 가 합니다. 서버도 무엇을 가리키는지 알지 못한 채 옮기기만 하므로,
> 형식을 짐작해 만들어 보내면 동작하지 않습니다.
>
> 이력에는 `/open f62dfe8a-…` 가 그대로 남습니다. 목록에서 읽기 어색한 값이라,
> 이력 화면에서 이 명령을 어떻게 보여 줄지는 따로 정하면 좋겠습니다.
...
#### 요약이 끝나면 원문이 남지 않습니다 (재생성 주의)

**요약이 성공하면 서버는 원문을 갖고 있지 않습니다**(slash-docs#3 · `slash-api#83`). 원문을
들고 있어야 하는 이유가 실패했을 때의 재시도뿐이라, 결과가 나온 순간 지웁니다.

그래서 그 작업을 다시 조회하면 `inputText` 자리에 **원문이 아니라 안내 문구**가 들어 있고,
문구는 어디서 요약했는지에 따라 다릅니다.

| 실행 위치 | `inputText` |
|---|---|
| `BROWSER` | `[브라우저에서 직접 요약 · 원문 1200자, 서버로 전송되지 않음]` |
| `BACKEND` | `[서버에서 요약 · 원문 1200자, 요약 후 저장하지 않음]` |

> **`inputText` 를 그대로 다시 접수하지 마세요.** 재생성 버튼이 이 값을 원문인 줄 알고
> `POST /api/v1/requests` 로 되돌려 보내면 "공백 제외 150자 이상 입력해 주세요"로 조용히
> 실패합니다. (`slash-web#67` 에서 실제로 재현된 증상입니다)

**`inputTextIsOriginal` 로 판단해 주세요.** 상세 응답에 **항상** 실리는 `boolean` 입니다.

| 값 | 뜻 | 화면 |
|---|---|---|
| `true` | `inputText` 가 사용자가 넣은 원문 그대로 | 재생성 가능 |
| `false` | 원문 대신 안내 문구 | 재생성 버튼을 비활성화하고, 원문을 다시 넣어 달라고 안내 |

- **문구를 하드코딩해 비교하지 마세요.** 위 두 문구는 예시이고, 실행 위치가 늘면 문구도
  늘어납니다(PC 실행기 요약이 열리면 세 번째가 생깁니다). 그때 프론트를 고치지 않아도
  되도록 두는 값입니다.
- **요약이 아닌 작업은 언제나 `true` 입니다.** `/status`·`/file`·`/weather` 처럼 원문을
  지우지 않는 작업이라 값을 따로 다룰 필요가 없습니다.
- **요약이 실패한 건도 `true` 입니다.** 실패하면 원문을 그대로 두므로 재생성이 됩니다 —
  사용자가 다시 누를 근거가 원문뿐이라 남깁니다.
- 목록(`GET /api/v1/tasks`)에는 없습니다. `inputText` 자체가 목록에 없어서 판단할 일이
  없습니다 — 한 줄을 펼쳐 상세를 받을 때 함께 옵니다.
...
>
> **`BROWSER` 는 브라우저가 스스로 제출한 요약에 옵니다.** WebGPU 를 지원하는 브라우저가
> `POST /api/v1/tasks/text-summary/browser-result` 로 결과를 제출하면 그 이력의
> `executionTarget` 이 `BROWSER` 입니다(위 §브라우저에서 이미 끝낸 요약 제출하기). 서버도
> PC 도 한 일이 아니니 PC 뱃지를 그리지 마세요.
...
> **요약(`TEXT_SUMMARY`)이 성공한 건만 예외입니다.** 원문 발췌 대신 **요약 결과의 앞부분**이
> 들어갑니다 — 원문을 갖고 있지 않기 때문입니다(위 §요약이 끝나면 원문이 남지 않습니다).
> 목록의 제목으로 쓰는 데는 달라질 것이 없습니다.
...
```json
{ "revealedAt": "2026-08-20T13:15:42.307+09:00" }
```

**파일을 실행하지 않습니다.** 그 파일이 있는 자리를 Finder·탐색기로 띄우기만 합니다.
돌려줄 내용이 없어 실행한 시각만 옵니다 — 성공 여부는 `status` 로 보시면 됩니다.

그 사이 파일이 옮겨졌거나 지워졌으면 `FILE_NOT_FOUND` (422) 로 옵니다. **목록에 보이던
파일도 누를 때는 없을 수 있습니다.** PC 실행기가 실행 직전에 다시 확인하기 때문입니다.

*(위는 실제로 받은 응답입니다. 파일을 찾아 여는 데 81ms, 없는 표식을 거절하는 데 38ms 였습니다.)*
